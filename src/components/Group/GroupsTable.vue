...
import { SearchGroupsQuery, SearchGroupsResponse } from '@/api/types/organization/Group';
...
const onFilter = (filters: { [key: string]: any }) => {
    pageFilters.value = new SearchGroupsQuery(pageFilters.value.page, pageFilters.value.page_size, filters.faculty_id, filters.name);
...
const deleteGroupAction = () => {
    deleteGroup(
        deleteGroupId.value,
        () => {
            toast.add({ severity: 'success', summary: 'Success', detail: 'Group deleted successfully', life: 5000 });
                ...
                <template #header>
                    <div class="flex flex-wrap align-items-center justify-content-between gap-2">
                        ...
                        <RouterLink to="/organization/group/create">
                            <Button icon="pi pi-plus" v-tooltip="'Create new group'" label="New" raised link />
                        </RouterLink>
                    </div>
                </template>
                ...
                <Column key="actions" header="Actions">
                    <template #body="slotProps">
                        <div class="flex flex-wrap gap-2">
                            <RouterLink :to="`/organization/group/${(slotProps.data as Group).group_id}`">
                                <Button icon="pi pi-eye" v-tooltip="'View group'" />
                            </RouterLink>
                            <CopyToClipboard :data="(slotProps.data as Group).group_id" tooltip="Copy ID"
                                icon="pi pi-copy" />
                            <RouterLink :to="`/organization/group/${(slotProps.data as Group).group_id}/edit`">
                                <Button icon="pi pi-pencil" v-tooltip="'Edit group'" />
                            </RouterLink>
                            <Button icon="pi pi-trash" v-tooltip="'Delete group'" class="p-button-danger"
                                @click="showDeleteGroupDialog((slotProps.data as Group).group_id as string)" />
                        </div>
                    </template>
                </Column>
            </DataTable>
            <Paginator v-model:page="pageFilters.page" v-model:totalRecords="metaInfo.total"
                :rows="pageFilters.page_size" @onPageChange="onPageChange"
                template="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown CurrentPageReport"
                :rowsPerPageOptions="[1, 5, 10, 20, 30]" />
        </div>
        <div class="col-2">
            <FilterPanel v-bind:filters="filters" v-bind:filterHandler="onFilter" title="Filters" searchName="Group" />
        </div>
    </div>

    <Dialog v-model:visible="deleteDialogVisible" modal header="Delete group" :style="{ width: '50%' }">
        <span class="p-text-secondary block mb-5">Are you sure you want to delete group {{ deleteGroupId }}?</span>
